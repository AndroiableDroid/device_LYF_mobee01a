...
#define ACCEL_CONVERT		(1.0f/1000000)//((GRAVITY_EARTH) / 16384) /* (4 * 1G / 2^16) */ // as to GRAVITY_HAL_UNIT in driver,qi.qi
#define CONVERT_ACCEL_X		(ACCEL_CONVERT)
#define CONVERT_ACCEL_Y		(ACCEL_CONVERT)
#define CONVERT_ACCEL_Z		(-ACCEL_CONVERT)

#define ACCEL_CONVERT_R		((GRAVITY_EARTH) / 16384) /* (4 * 1G / 2^16) */
#define CONVERT_ACCEL_RX	(-ACCEL_CONVERT_R)
#define CONVERT_ACCEL_RY	(ACCEL_CONVERT_R)
#define CONVERT_ACCEL_RZ	(-ACCEL_CONVERT_R)
	...
	while (count && mInputReader.readEvent(&event)) {
		...
		if (type == EV_ABS) {
			...
			if (event->code == EVENT_TYPE_ACCEL_X) {
				...
				mPendingEvent.data[0] = value * CONVERT_ACCEL_RX;
			} else if (event->code == ABS_RY) {
				mPendingEvent.data[1] = value * CONVERT_ACCEL_RY;
			} else if (event->code == ABS_RZ) {
				mPendingEvent.data[2] = value * CONVERT_ACCEL_RZ;
			}
		} else if (type == EV_SYN) {
			switch (event->code){
				case SYN_TIME_SEC:
